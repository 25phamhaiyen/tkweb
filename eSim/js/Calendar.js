...
document.addEventListener('click', (event) => {
    // Kiểm tra nếu lần nhấp không nằm trong phần tử lịch và cũng không phải nút chọn ngày
    if (!calendar.contains(event.target) && event.target !== selectStartDateBtn && event.target !== selectEndDateBtn) {
        calendar.style.display = 'none'; // Ẩn lịch
    }
});
